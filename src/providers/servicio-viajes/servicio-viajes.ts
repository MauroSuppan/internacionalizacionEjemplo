...
export class ServicioViajesProvider {
  ...
  traerViajes(): Observable<Viaje[] | any[]> {
    ...
    let coleccionTipadaFirebase = this.objFirebase.collection<Viaje>(this.tablaViajes,ref => ref.orderBy("fechaRegistro", "desc") );
...
*/
  ...
  // debería hacer unsuscribe al finalizar la carga de datos?
  traerViajesFiltrados(campo: string, criterio, filtro: string): Observable<Viaje[] | any[]> {
  ...
  //HECHO POR MAURO DESCOMNETAR, CUANDO FUNCIONE
       // let coleccionTipadaFirebase = this.objFirebase.collection<Viaje>(this.tablaViajes, ref => ref.where(campo, criterio, filtro).orderBy("fechaRegistro", "desc"));
    let ListadoViajesObservable = coleccionTipadaFirebase.valueChanges();
    return ListadoViajesObservable;
    /*let ob = ListadoViajesObservable.subscribe(x => {
      console.info("Conexión correcta con Firebase. Trayendo todos los viajes: ", x);
  ...
  geoCoding(direccion: string): Promise<any> {
    ...
    geoCodingLatLong(lat:number, lng:number): Promise<any> {
      let url: string = 'https://maps.googleapis.com/maps/api/geocode/json?latlng=' + lat +','+ lng + '&key=' + environment.googleMapsApiKey;
    return this.http.get(url).toPromise();
  }
