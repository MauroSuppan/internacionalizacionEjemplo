...
export class LoginPage {
  ...
  loginFields: { correo: string, clave: string } = {
    correo: 'gmercado@gmail.com',  // hardcodeado para hacer más rápido los test
    clave: '123'
  };
  ...
  setLog(i: number) {
    switch (i) {
      case 1:
        this.loginFields.correo = "gmercado@gmail.com";
  ...
  mostrarUsuarios() {
    ...
    let actionSheet = this.actionSheetCtrl.create({
      ...
      cssClass: 'actSheet',
      buttons: [
        { text: 'Gabriel Mercado', handler: () => {this.setLog(1);}},
    ...
    let ob = this.servicioUsuarios.traerUsuarios().subscribe((arr) => {
      ...
      if (user !== undefined && user.activo == 1) {
        ...
        let toast = this.toastCtrl.create({
          message: ( user !== undefined && user.activo == 0 ? this.language.usuarioBaja : this.language.accesoDenegado),
          duration: 4000,
